...
var columns=[
  ...
  {name:"Name",selector:row=>row.name,width:"16%",sortable:true},
  {name:"Email",selector:row=>row.email,width:"24%",sortable:true},
  {name:"Role",width:"12%",selector:row=>(
    <>
        {row.Role=="Admin"?<Pending>Admin</Pending>:null}
        {row.Role=="Customer"?<Delivered>Customer</Delivered>:null}
        {row.Role=="dispatched"?<Dispatched>dispatched</Dispatched>:null}
    </>
  )},
  {name:'Actions',width:"30%",selector:row=>(
    <>
      <Actions>
      <View onClick={()=>navigate(`/user_details/${row.id}`)}>View</View>
      <DispatchedBtn onClick={()=>handle_delete(row.id)}> Deleted</DispatchedBtn>
      {row.Role=="Admin"?<DeliveredBtn onClick={()=>handle_make_customer(row.id)}>Customer</DeliveredBtn>:<DeliveredBtn onClick={()=>handle_make_admin(row.id)}>Admin</DeliveredBtn>}
  ...
  return (
    <div>
      ...
      <div style={{ height: 470, width: '100%',overflow:'auto'}}>
          <DataTable
          data={row}
          columns={columns}
          selectableRows
