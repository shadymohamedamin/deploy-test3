...
`;
var columns=[
  {name:"ID",selector:row=>row.id,width:"22%",component: StyledHeader,},
  {name:"Name",selector:row=>row.name,width:"13%",sortable:true},
  {name:"Amount",selector:row=>"$"+row.amount,width:"12%",sortable:true},
  {name:"Status",width:"12%",selector:row=>(
    <>
  ...
  {name:'Date',selector:row=>row.dat,width:"15%",sortable:true},
  {name:'Actions',width:"30%",selector:row=>(
    <>
      <Actions>
      <DispatchedBtn onClick={()=>handle_dispatch(row)}>Dispatched</DispatchedBtn>
      <DeliveredBtn onClick={()=>handle_delivered(row)}>Delivered</DeliveredBtn>
